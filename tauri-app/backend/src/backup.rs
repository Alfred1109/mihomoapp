...
pub async fn backup_config() -> Result<()> {
    ...
    // 复制配置文件到备份目录
    fs::copy(&config_path, &backup_path).context("Failed to backup config file")?;
...
/// 恢复指定的备份配置
pub async fn restore_config(backup_filename: &str) -> Result<RestoreResult> {
    let backup_dir = get_backup_dir()?;
    let backup_path = backup_dir.join(backup_filename);
    ...
    if !backup_path.exists() {
        return Ok(RestoreResult {
            ...
            message: format!("备份文件不存在: {}", backup_filename),
    ...
    // 检查服务是否在运行
    let service_running = crate::mihomo::is_mihomo_running().await;

    // 在恢复前先备份当前配置
    if config_path.exists() {
        let temp_backup = format!(
            "config.yaml.backup.before_restore_{}",
            chrono::Utc::now().format("%Y%m%d_%H%M%S")
        );
        let temp_backup_path = backup_dir.join(&temp_backup);
        fs::copy(&config_path, &temp_backup_path).context("Failed to backup current config")?;
    }
    ...
    Ok(RestoreResult {
        ...
        service_running,
        message,
        requires_restart: service_running,
    })
}
...
pub async fn rename_backup(old_filename: &str, new_label: &str) -> Result<String> {
    ...
    fs::rename(&old_path, &new_path).context("Failed to rename backup file")?;
...
fn get_backup_dir() -> Result<PathBuf> {
    // 使用统一的平台配置系统
    crate::platform_config::PlatformPaths::backup_dir()
}
