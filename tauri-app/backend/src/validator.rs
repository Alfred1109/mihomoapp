...
fn validate_ports(config: &serde_json::Value, errors: &mut Vec<String>, warnings: &mut Vec<String>) {
    ...
    for field in port_fields {
        if let Some(port) = config.get(field).and_then(|v| v.as_u64()) {
            ...
            // 检查常用端口
            if port < 1024 {
                warnings.push(format!("{} 使用了系统保留端口 {}, 可能需要管理员权限", field, port));
            }
        }
    }
}
...
fn validate_proxies(config: &serde_json::Value, errors: &mut Vec<String>, warnings: &mut Vec<String>) {
    if let Some(proxies) = config.get("proxies").and_then(|v| v.as_array()) {
        ...
        for (idx, proxy) in proxies.iter().enumerate() {
            ...
            if let Some(proxy_type) = proxy.get("type").and_then(|v| v.as_str()) {
                let valid_types = vec!["ss", "ssr", "vmess", "vless", "trojan", "snell", "http", "socks5"];
                if !valid_types.contains(&proxy_type) {
                    warnings.push(format!("代理节点 #{} 使用了不常见的类型: {}", idx, proxy_type));
                }
            }
        }
    } else {
...
fn validate_proxy_groups(config: &serde_json::Value, errors: &mut Vec<String>, warnings: &mut Vec<String>) {
    if let Some(groups) = config.get("proxy-groups").and_then(|v| v.as_array()) {
        ...
        let proxy_names: HashSet<String> = if let Some(proxies) = config.get("proxies").and_then(|v| v.as_array()) {
            proxies.iter()
        ...
        for (idx, group) in groups.iter().enumerate() {
            ...
            if let Some(group_type) = group.get("type").and_then(|v| v.as_str()) {
                let valid_types = vec!["select", "url-test", "fallback", "load-balance", "relay"];
